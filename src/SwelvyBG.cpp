...
bool SwelvyBG::init() {
    ...
    for (auto layer : std::initializer_list<std::pair<ccColor3B, const char*>> {
        ...
        for (int i = 0; i < repeatCount; i += 1) {
            ...
            auto sprite = SwelveNode::createWithSpriteFrame(frame);
            ...
            sprite->speed = speed;
            sprite->repeatCount = repeatCount;
...
void SwelvyBG::updateSpritePosition(float dt) {
    auto speed = base_cast<SwelveNode*>(this)->speed;
    auto repeatCount = base_cast<SwelveNode*>(this)->repeatCount;
